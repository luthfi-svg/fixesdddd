...
module.exports = async function handler(req, res) {
    ...
    try {
        checkRateLimit(req, 'stalktt');
        await assertSiteOpen();
        const body =
            typeof req.body === 'string'
                ? JSON.parse(req.body || '{}')
                : req.body || {};
        const username = body.username || body.user || body.url || body.tiktokUrl;
        if (!username) {
            const error = new Error('Username TikTok wajib diisi.');
            error.statusCode = 400;
            throw error;
        }
        const result = await stalkTikTok(username);
        await recordRequest({ req, platform: 'stalktt', success: Boolean(result.status), durationMs: Date.now() - started, error: result.status ? '' : result.message });
        if (!result.status) await recordError({ req, platform: 'stalktt', error: new Error(result.message || 'Gagal mengambil profil.') });
        return res.status(result.status ? 200 : 400).json(result);
    } catch (error) {
        const status = error.statusCode || (error.code === 'SITE_CLOSED' ? 503 : error.code === 'RATE_LIMITED' ? 429 : 500);
        if (error.code !== 'RATE_LIMITED') {
            await recordRequest({ req, platform: 'stalktt', success: false, durationMs: Date.now() - started, error: error.message });
            await recordError({ req, platform: 'stalktt', error });
        }
        if (error.retryAfter) res.setHeader('Retry-After', String(error.retryAfter));
        console.error('[STALKTT API]', error);
        return res.status(status).json({ status: false, message: error.message || 'Terjadi kesalahan pada server.' });
    }
};
