  ...
  try {
    ...
    if (['start', 'help'].includes(command)) {
      await reply(
        ...
        [
          ...
          '/notice check',
          '',
          '/logs - lihat visitor terakhir',
          '/logs clear - hapus visitor logs',
          '',
          isAdmin(fromId)
            ? 'Akses admin: AKTIF'
    ...
    if (command === 'logs') {
      ...
      if (action === 'clear') {
        ...
        await reply(
          chatId,
          'Visitor logs berhasil dihapus. 🗑️'
        );
      ...
      const lines = [
        `VISITOR LOGS — ${logs.length} tersimpan`,
        ''
      ];

      recent.forEach((item, index) => {
        lines.push(
          `${index + 1}. ${item.ip} • ${item.device} • ${item.os}`,
          `   ${item.browser} • ${item.timestamp}`,
          `   ${item.path}`,
          ''
        );
      });
