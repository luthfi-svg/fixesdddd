...
async function setAioFeature(platform, enabled) {
  assertConfigured();
  const key = String(platform || '').trim().toLowerCase();

  if (key === 'stalktt') {
    throw new Error('TikTok Stalker tidak termasuk kontrol AIO.');
  }

  if (!Object.prototype.hasOwnProperty.call(DEFAULT_AIO_FEATURES, key)) {
    throw new Error(`Platform AIO tidak dikenal: ${key}`);
  }

  return updateState((next) => {
    next.aio = normalizeAio(next.aio);
    next.aio.features[key] = Boolean(enabled);
    next.aio.enabled = Object.values(next.aio.features).some(Boolean);
    return next;
  });
}

async function setAioEnabled(enabled) {
  assertConfigured();

  return updateState((next) => {
    next.aio = normalizeAio(next.aio);
    next.aio.enabled = Boolean(enabled);
    Object.keys(next.aio.features).forEach((key) => {
      next.aio.features[key] = Boolean(enabled);
    });
    return next;
  });
