...
function normalizeVisitorLog(value) {
  if (!value || typeof value !== 'object') return null;

  return {
    id: String(value.id || Date.now()),
    ip: typeof value.ip === 'string' ? value.ip : 'unknown',
    device: typeof value.device === 'string' ? value.device : 'Unknown',
    os: typeof value.os === 'string' ? value.os : 'Unknown',
    browser: typeof value.browser === 'string' ? value.browser : 'Unknown',
...
async function getGistFile(fileName, fallback) {
  const gist = await getGist();
  const file = gist?.files?.[fileName];
  ...
  try {
    ...
    throw new Error(`File "${fileName}" di GitHub Gist bukan JSON yang valid.`);
  }
}
...
async function getVisitorLogs() {
  const fileName = config.control.visitorLogFileName || 'visitor-logs.json';
  const parsed = await getGistFile(fileName, []);

  if (!Array.isArray(parsed)) {
    throw new Error(`File "${fileName}" harus berupa JSON array.`);
  }

  return parsed
    .map(normalizeVisitorLog)
    .filter(Boolean);
}

async function addVisitorLog(entry) {
  assertConfigured();

  const fileName = config.control.visitorLogFileName || 'visitor-logs.json';
  const logs = await getVisitorLogs();
  const normalized = normalizeVisitorLog(entry);
