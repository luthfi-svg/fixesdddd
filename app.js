...
function normalizeAioResult(data) {
  const raw = data?.data || {};
  const root =
  ...
  const downloads = Array.isArray(root.downloads)
    ? root.downloads
    : Array.isArray(root.links)
      ? root.links.map((item, index) => ({
          type: item?.type || 'video',
          quality: item?.quality || item?.label || `Media ${index + 1}`,
          format: item?.format || 'MP4',
          url: item?.url || item?.download || item?.href || null
  ...
  const normalized = {
    title: root.title || root.name || root.caption || 'AIO Media',
    cover: root.cover || root.thumbnail || root.thumbnailUrl || root.image || null,
    author: root.author || root.user || {},
    statistics: root.statistics || root.stats || {},
    duration: Number(root.duration_seconds || root.duration || 0),
    video: directVideo,
    audio: directAudio,
    images: Array.isArray(root.images) ? root.images : [],
    downloads,
    platform: `aio:${data?.source_platform || 'media'}`
  };

  if (!normalized.downloads.length && directVideo) {
  ...
  if (!normalized.downloads.length && directAudio) {
    normalized.downloads.push({
      type: 'audio',
...
async function fetchStatus() {
  try {
    ...
    state.aio = data.aio || null;
    const aioTab = els.tabs.find((tab) => tab.dataset.platform === 'aio');
    if (aioTab) {
      const aioEnabled = data.aio?.enabled !== false;
      aioTab.classList.toggle('disabled', !aioEnabled);
      aioTab.setAttribute('aria-disabled', String(!aioEnabled));
      aioTab.title = aioEnabled
        ? 'AIO Downloader'
        : 'AIO Downloader sedang dinonaktifkan';
    }
